...
prefix = "** " <> showStack Stack.callStack <> ": "

pshow :: Show a => a -> Text
pshow = Text.strip . Text.pack . Show.Pretty.ppShow

showStack :: Stack.CallStack -> Text
showStack stack = case reverse (Stack.getCallStack stack) of
    (_, srcloc) : _ -> Text.pack fname <> ":" <> Text.pack (show lineNumber)
        where
        fname = strip (Stack.srcLocFile srcloc)
        lineNumber = Stack.srcLocStartLine srcloc
    [] -> "<no-caller>"
    where
    strip ('.':'/':s) = s
    strip s = s
